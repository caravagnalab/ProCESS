...
#ifndef __PROCESS_MUTATION_ENGINE__
#define __PROCESS_MUTATION_ENGINE__
...
class MutationEngine
{
    ...
    template <typename MUTATION_TYPE> void show_timed_exposures() const
    {
        ...
        while (coeffs_it != timed_exposures.end()) {
            ...
            if (next_it == timed_exposures.end()) {
                ...
                Rcpp::Rcout << "     [" << coeffs_it->first << ", " << next_it->first
                            << "[: ";
            }
  ...
  public:
    ...
    inline void add_exposure(const Rcpp::List &exposure) { add_exposure(0, exposure); }
    ...
    inline bool get_infinite_sites_model() const { return m_engine.infinite_sites_model; }
    ...
    static MutationEngine build_MutationEngine(
        const std::string &directory, const std::string &reference_source,
        const std::string &SBS_signatures_source,
