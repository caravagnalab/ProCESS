...
struct GermlineSubject
{
    ...
    {
        archive & name & population & super_population & gender;
    }
    ...
    inline static GermlineSubject load(ARCHIVE &archive)
    {
        ...
        archive & germline_subject.name & germline_subject.population &
            germline_subject.super_population & germline_subject.gender;
...
class GermlineStorage
{
  ...
  public:
    ...
    inline std::filesystem::path get_path() const { return directory; }
...
class GenomicDataStorage
{
    ...
    collect_signatures_download_list(
        std::list<std::pair<std::string, std::filesystem::path>> &download_list) const
    {
        ...
        if (!is_an_URL(source)) {
            Rcpp::stop("Signature file \"" + to_string(source) + "\" does not exists.");
        }
