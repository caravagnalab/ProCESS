...
using SIDSpec = CLONES::Mutations::MutationSpec<CLONES::Mutations::SID>;
...
struct MutationEngineSetup
{
    ...
    MutationEngineSetup(
        const std::string &description, const std::filesystem::path &directory,
        const std::string &reference_url, const std::string &SBS_signatures_url,
        const std::string &indel_signatures_url, const std::string &drivers_url,
        const std::string &passenger_CNAs_url, const std::string &germline_url)
        : description(description), directory(directory), reference_url(reference_url),
          SBS_signatures_url(SBS_signatures_url),
          indel_signatures_url(indel_signatures_url), drivers_url(drivers_url),
...
std::map<std::string, MutationEngineSetup> supported_setups{
    {"demo",
     {"A demonstrative set-up", "demo",
      "https://ftp.ensembl.org/pub/grch37/release-111/fasta/homo_sapiens/dna/"
...
GenomicDataStorage setup_storage(
...
inline const std::string &first_if_empty(const std::string &dst, const std::string &value)
{
    return (dst.size() > 0 ? dst : value);
}

GenomicDataStorage setup_storage(
    const std::string &setup_code, const std::string &directory,
    ...
    std::shared_ptr<Account> COSMIC_account = nullptr)
{
    ...
    return setup_storage(
        first_if_empty(directory, code_directory),
        first_if_empty(reference_source, code_it->second.reference_url),
        first_if_empty(SBS_signatures_source, code_it->second.SBS_signatures_url),
...
Rcpp::List MutationEngine::get_supported_setups()
{
    ...
    CharacterVector code_names(supported_setups.size()),
        code_descrs(supported_setups.size());
...
get_region_to_avoid(const GenomicDataStorage &storage)
{
    ...
    if (std::filesystem::exists(drivers_path)) {
        auto driver_storage = CLONES::Mutations::DriverStorage::load(drivers_path);
...
CLONES::Mutations::ContextIndex<ABSOLUTE_GENOTYPE_POSITION>
...
CLONES::Mutations::RSIndex build_rs_index(const GenomicDataStorage &storage,
                                         const size_t max_motif_size,
                                         const size_t max_repetition_storage,
                                         const bool &quiet)
{
...
std::map<CLONES::Mutations::ChromosomeId, size_t> get_num_of_alleles(
    const CLONES::Mutations::ContextIndex<ABSOLUTE_GENOTYPE_POSITION> &context_index,
...
template <
    ...
    std::enable_if_t<std::is_base_of_v<CLONES::Mutations::MutationType, MUTATION_TYPE>,
                     bool> = true>
std::map<std::string, CLONES::Mutations::Signature<MUTATION_TYPE>>
...
CLONES::Mutations::GenomicRegion get_CNA_region(const CLONES::IO::CSVReader::CSVRow &row,
                                               const size_t &row_num)
{
    ...
    try {
        ...
        Rcpp::stop("Unknown begin specification " + row.get_field(1) + " in row number " +
    ...
    try {
        ...
        Rcpp::stop("Unknown end specification " + row.get_field(2) + " in row number " +
                   std::to_string(row_num) + ".");
    }
...
load_passenger_CNAs(const std::filesystem::path &CNAs_csv, const std::string &tumour_type)
{
    ...
    for (const auto &row : csv_reader) {
        ...
        if ((tumour_type == "") || (row.get_field(5) == tumour_type)) {
            ...
            try {
                ...
                Rcpp::stop("Unknown major specification " + major + " in row number " +
                           std::to_string(row_num) + ".");
            }
            ...
            try {
                ...
                Rcpp::stop("Unknown minor specification " + major + " in row number " +
                           std::to_string(row_num) + ".");
            }
        }
    ...
    return std::vector<CLONES::Mutations::CNA>(CNAs.begin(), CNAs.end());
}
...
MutationEngine::MutationEngine(
    const std::shared_ptr<Account> &COSMIC_account, const std::string &setup_name,
    const std::string &directory, const std::string &reference_source,
    const std::string &SBS_signatures_source, const std::string &indel_signatures_source,
...
MutationEngine::MutationEngine(
    const std::shared_ptr<Account> &COSMIC_account, const std::string &directory,
    const std::string &reference_source, const std::string &SBS_signatures_source,
    const std::string &indel_signatures_source, const std::string &drivers_source,
...
struct DummyTest
{
    template <typename RCPP_TYPE> void validate(const RCPP_TYPE &value) const {}
};

struct TestNonNegative
{
    template <typename RCPP_TYPE> void validate(const RCPP_TYPE &value)
    {
...
std::map<std::string, VALUE> get_map(const Rcpp::List &list)
{
...
std::shared_ptr<Account> extract_COSMIC_account(const SEXP &COSMIC_account_data)
{
    ...
    return std::make_shared<Account>(
        Rcpp::as<std::string>(list_account_data["email"]),
        Rcpp::as<std::string>(list_account_data["password"]));
}

MutationEngine MutationEngine::build_MutationEngine(
    const std::string &directory, const std::string &reference_source,
    const std::string &SBS_signatures_source, const std::string &indel_signatures_source,
    const std::string &drivers_source, const std::string &passenger_CNAs_source,
...
Rcpp::List MutationEngine::get_available_tumour_type(const std::string &setup_code)
{
    ...
    CLONES::IO::CSVReader csv_reader(storage.get_passenger_CNAs_path(), true, '\t');
    for (const auto &row : csv_reader) {
        if (row.size() < 6) {
            Rcpp::stop("The CNA CSV must contains at least 6 columns");
        }
...
const CLONES::Mutations::DriverStorage::MutationEntry &
get_mutation_from_name(const CLONES::Mutations::DriverStorage &driver_storage,
                       const std::string &tumour_type, const std::string &mutation_code)
{
    ...
    if (tumour_type != "") {
        if (found->second.tumour_types.count(tumour_type) == 0) {
            ...
            oss << "\"" << mutation_code << "\" is not a driver for "
                << "the specified tumor type \"" << tumour_type << "\"." << std::endl;
...
std::pair<CLONES::Mutations::AlleleId, std::string>
get_mutation_spec_from_list(const CLONES::Mutations::DriverStorage &driver_storage,
                            const Rcpp::List &SID_spec, const size_t index)
{
    ...
    if (spec_size > 2 || spec_size == 0) {
        Rcpp::stop("The " + ordtostr(index) + " element in the driver mutation list" +
    ...
    CLONES::Mutations::AlleleId allele_id = RANDOM_ALLELE;
    ...
    if (spec_size > 1) {
        ...
        allele_id = Rcpp::as<CLONES::Mutations::AlleleId>(SID_spec[1]);
    }
                               ...
                               std::map<SIDSpec, std::string> &mutant_drivers)
{
    ...
    if (found != mutant_drivers.end()) {
        ...
        oss << " has been declared as driver of mutant " << mutant_name
    ...
    std::map<SIDSpec, std::string> &mutant_drivers)
{
    ...
    application_order.push_back(CLONES::Mutations::DriverMutations::SID_TURN);
}

void get_mutation_spec(
    const std::string &mutant_name, std::list<SIDSpec> &c_sids,
    ...
    const std::string &tumour_type, const Rcpp::List &rcpp_list, const size_t &index)
{
    ...
    case STRSXP:
    {
        ...
        auto mutation_entry =
            get_mutation_from_name(driver_storage, tumour_type, mutation_code);
    ...
    {
        auto mutation_data =
            get_mutation_spec_from_list(driver_storage, mutation, index + 1);

        auto mutation_entry =
            get_mutation_from_name(driver_storage, tumour_type, mutation_data.second);

        insert_among_drivers({mutation_data.first, mutation_entry.mutation},
                             mutation_data.second, mutant_name, c_sids, application_order,
                             mutant_drivers);
    ...
    case S4SXP:
    {
        ...
        if (s4obj.is("Rcpp_Mutation")) {
            ...
            if (sid_spec.ref != "?") {
                ...
                insert_among_drivers(sid_spec, mutation_code, mutant_name, c_sids,
        ...
        if (s4obj.is("Rcpp_CNA")) {
            ...
            c_cnas.push_back(static_cast<const CLONES::Mutations::CNA &>(cna));

            application_order.push_back(CLONES::Mutations::DriverMutations::CNA_TURN);

            return;
        }
        if (s4obj.is("Rcpp_WholeGenomeDoubling")) {
            application_order.push_back(CLONES::Mutations::DriverMutations::WGD_TURN);
    ...
    Rcpp::stop("The " + ordtostr(index + 1) + " element in the driver mutation list" +
               " is not an mutation specification");
}
...
CLONES::Mutations::PassengerRates get_passenger_rates(const Rcpp::List &list)
{
    CLONES::Mutations::PassengerRates p_rates;
    ...
    for (size_t i = 0; i < list_size; ++i) {
        if (names[i] == "SNV") {
            ...
            p_rates.indel =
                get_non_negative(list[i], ": indel rates must be non-negative");
        } else {
...
std::map<std::string, CLONES::Mutations::PassengerRates>
get_epistate_passenger_rates(const Rcpp::List &list)
{
    std::map<std::string, CLONES::Mutations::PassengerRates> ep_rates;
...
void error_if_chr_missing(
    const std::map<CLONES::Mutations::ChromosomeId, std::list<SIDSpec>> &missing_chr)
{
    if (missing_chr.size() > 0) {
        ...
        Rcpp::stop((counter > 1 ? "SIDs " : "SID ") + oss.str() +
    ...
    std::map<SIDSpec, std::string> &mutant_drivers)
{
    ...
    std::map<CLONES::Mutations::ChromosomeId, std::list<SIDSpec>> missing_chr;

    std::string ref_str;

    size_t checked{0};
    IndexedReader<ChromosomeData<Sequence>> chr_reader(fasta_filename, progress_bar);

    for (auto &sid : SIDs) {
        const auto chr_name = CLONES::Mutations::GenomicPosition::chrtos(sid.chr_id);
        if (chr_reader.read(ref_str, chr_name, sid.position - 1, sid.ref.size())) {
            if (ref_str.size() == 0) {
                std::ostringstream oss;
            ...
            if (sid.ref == "?") {
                ...
                insert_mutant_driver_code(sid, mutation_code, mutant_name,
                                          mutant_drivers);
            } else {
                if (sid.ref != ref_str) {
                    std::ostringstream oss;
                    oss << "The specified reference of " << sid
                        << " does not match the reference sequence (" << ref_str << ")."
                        << std::endl;
                    throw std::out_of_range(oss.str());
                }
            }
            ...
            progress_bar.set_progress(
                static_cast<unsigned int>((100 * (++checked)) / SIDs.size()));
        } else {
                                ...
                                const Rcpp::List &drivers)
{
    ...
    auto mutant_drivers =
        get_mutation_lists(mutant_name, c_sids, c_cnas, application_order, driver_storage,
                           reverse_driver_storage, tumour_type, drivers);

    retrieve_missing_references(mutant_name, reverse_driver_storage,
                                storage.get_reference_path(), c_sids, mutant_drivers);
...
{
    std::map<std::string, CLONES::Mutations::PassengerRates> epi_rates;
...
Rcpp::List MutationEngine::get_SNV_signatures_dataframe() const
{
    ...
    return read_delim(to_string(storage.get_signatures_path<CLONES::Mutations::SBSType>()),
                      Rcpp::_["quote"] = "");
}
...
Rcpp::List MutationEngine::get_indel_signatures_dataframe() const
{
    ...
    return read_delim(to_string(storage.get_signatures_path<CLONES::Mutations::IDType>()),
                      Rcpp::_["quote"] = "");
}
...
Rcpp::List MutationEngine::get_genome_info() const
{
    ...
    for (const auto &[chr_id, chromosome] : chromosomes) {
        chr_names[i] = CLONES::Mutations::GenomicPosition::chrtos(chr_id);
...
std::ostream &show_list(std::ostream &os, ITERATOR it, ITERATOR last,
        ...
        case DriverMutations::SID_TURN:
        {
            ...
            if (found != driver_reverse_map.end()) {
                os << found->second << (found->second.size()>0?" ":"")
                   << "(" << static_cast<CLONES::Mutations::SID>(*SID_it)
                   << ")";
            } else {
                os << static_cast<CLONES::Mutations::SID>(*SID_it);
            }
...
Rcpp::List MutationEngine::get_species_info(const CLONES::Mutations::MutationalProperties& m_properties)
{
    using namespace Rcpp;
    using namespace CLONES::Mutations;
...
{
    using namespace Rcpp;
    Rcout << "MutationEngine" << std::endl << " Passenger rates";
    ...
    for (const auto &[mutant_name, driver_mutations] :
         m_properties.get_driver_mutations()) {
    ...
    show_timed_exposures<CLONES::Mutations::SBSType>();
    Rcout << std::endl;
    show_timed_exposures<CLONES::Mutations::IDType>();
    Rcout << std::endl;
}
...
void warning_function(const CLONES::WarningType type, const std::string message)
{
    ...
    switch (type) {
    case CLONES::WarningType::NO_MUT_FOR_CONTEXT:
        oss << " Decrease `MutationEngine`'s "
            << "parameter `context_sampling`." << std::endl;
        break;
    case CLONES::WarningType::NO_MUT_FOR_RPATTERN:
...
void MutationEngine::reset(const bool full, const bool quiet)
{
    ...
    std::map<MutationType::Type, std::map<CLONES::Time, MutationalExposure>>
        timed_exposures;
    ...
    if (!full) {
        ...
        timed_exposures[MutationType::Type::SBS] =
            const_m_engine.get_timed_exposures(MutationType::Type::SBS);
        timed_exposures[MutationType::Type::INDEL] =
            const_m_engine.get_timed_exposures(MutationType::Type::INDEL);
    }
    ...
    auto passenger_CNAs =
        load_passenger_CNAs(storage.get_passenger_CNAs_path(), tumour_type);
    ...
    m_engine = CLONES::Mutations::MutationEngine(
