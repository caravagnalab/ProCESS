...
GermlineSubject::GermlineSubject(const std::string &name, const std::string &population,
    ...
    : name(name), population(population), super_population(super_population),
      gender(gender)
{}
...
Rcpp::List GermlineSubject::get_dataframe() const
{
    ...
    return DataFrame::create(_["sample"] = sample, _["pop"] = pop,
                             _["super_pop"] = super_pop, _["gender"] = gender);
}
    ...
    : directory(directory)
{
    if (!std::filesystem::exists(directory)) {
        Rcpp::stop("Designed germline mutations directory \"" + to_string(directory) +
                   "\" does not exist.");
    }
    ...
    if (!std::filesystem::exists(get_file())) {
        Rcpp::stop("Designed germline mutations directory \"" + to_string(directory) +
                   "\" does not " + "contains the file \"germline.csv\".");
    }
    if (!std::filesystem::exists(get_population_file())) {
        Rcpp::stop("Designed germline mutations directory \"" + to_string(directory) +
                   "\" does not " + "contains the file \"population.csv\".");
    }
    if (!std::filesystem::exists(get_population_descriptions_file())) {
        Rcpp::stop("Designed germline mutations directory \"" + to_string(directory) +
                   "\" does not " + "contains the file \"population_descr" +
                   "iptions.csv\".");
    }
    if (!std::filesystem::exists(get_alleles_file())) {
        Rcpp::stop("Designed germline mutations directory \"" + to_string(directory) +
                   "\" does not " + "contains the file \"alleles_per_chr." + "csv\".");
    }
}
...
Rcpp::List GermlineStorage::get_population_df() const
{
    ...
    return read_csv(_["file"] = to_string(get_population_file()), _["quote"] = "",
                    _["header"] = true, _["sep"] = "\t");
}
...
GermlineStorage::get_alleles_per_chromosome(const std::string &gender) const
{
    ...
    for (const auto &row : csv_reader) {
        ...
        alleles_per_chromosome.insert({chr_id, std::stoul(row.get_field(index))});
    }
...
GermlineSubject GermlineStorage::get_subject(const std::string &subject_name) const
{
    ...
    for (const auto &row : csv_reader) {
        if (row.get_field(0) == subject_name) {
            return {row.get_field(0), row.get_field(1), row.get_field(2),
                    row.get_field(3)};
        }
    }

    Rcpp::stop("Germline subject \"" + subject_name + "\" not available.");
}
...
Rcpp::List Rcpp_wrap_download_list(
    const std::list<std::pair<std::string, std::filesystem::path>> &download_list)
{
    using namespace Rcpp;
    ...
    for (const auto &[url, dest_filename] : download_list) {
        wrapped_list[i] =
            List::create(_["url"] = url, _["dest_filename"] = to_string(dest_filename));
        ++i;
    }
...
void download_COSMIC(
    const std::shared_ptr<Account> &COSMIC_account,
    const std::list<std::pair<std::string, std::filesystem::path>> &download_list)
{
    using namespace Rcpp;
    ...
    if (COSMIC_account.get() == nullptr) {
        stop("Since April 2nd, 2025, COSMIC site (https://cancer.sanger.ac.uk/cosmic/)\n"
             "requires an account. Create an account, download SBS and ID mutation "
             "signature\n"
             "files, and pass them as parameters to `MutationEngine()` call. In "
             "alternative,\n"
             "provide COSMIC account details to `MutationEngine()` and let ProCESS "
             "download\n"
             "the signature files.\n");
    }

    const std::string r_code =
        "function(username, password, d_list) {\n"
...
void GenomicDataStorage::retrieve_signatures(
    const std::shared_ptr<Account> &COSMIC_account) const
{
...
std::filesystem::path
GenomicDataStorage::get_destination_path(const std::string &url) const
{
    try {
...
std::filesystem::path GenomicDataStorage::retrieve_reference() const
{
    if (!is_an_URL(reference_src) && !std::filesystem::exists(reference_src)) {
        Rcpp::stop("Designed reference genome file \"" + reference_src +
                   "\" does not exists.");
    }
...
void GenomicDataStorage::retrieve_drivers() const
{
    ...
    if (!is_an_URL(drivers_src) && !std::filesystem::exists(drivers_src)) {
        Rcpp::stop("Designed driver mutations file \"" + drivers_src +
                   "\" does not exists.");
    }
...
void GenomicDataStorage::retrieve_passenger_CNAs() const
{
    ...
    if (!is_an_URL(passenger_CNAs_src)) {
        Rcpp::stop("Designed passenger CNAs file \"" + passenger_CNAs_src +
                   "\" does not exists.");
    }
...
void GenomicDataStorage::retrieve_germline() const
{
    ...
    if (!is_an_URL(germline_src)) {
        Rcpp::stop("Designed germline directory \"" + germline_src +
                   "\" does not exists.");
    }
