...
ProCESS/CLONES can simulate genomic mutations on the cells represented in
...
declated to the mutation engine before labeling the sample forest.
...
In order to simulate passenger mutations of a given type, we need to specify a
...
sample forest by mutations.

Since the mutation engine has be configured to deal with the
simulation performed in Section "_Two populations with
epigenetic state_" of `vignette("sampling")`,
we can load its sample forest from the file `"sample_forest.sff"`
...
`DescendantForest$get_samples_info()`: it returns a data frame containing
...
The method `MutationEngine$get_bulk_allelic_fragmentation()` returns a data frame
...
Instead, the method `MutationEngine$get_cell_allelic_fragmentation()` returns
the allelic fragmentation per cell.
