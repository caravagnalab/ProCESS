...
> Refer to the [article about sequencing](sequencing.html)
...
The sequencing simulation supports a sample partition
analogous to [FACS](https://www.sinobiological.com/category/fcm-facs-facs).
Users can provide a labelling function for the sampled cells by
using the `simulate_seq()` parameter `cell_labelling`.
This function must take as input an object of the class `SampledCell` and
...
Let us assume that the phylogenetic forest `phylo_forest`
was built as detailed in [this article](mutations.html).
...
The sampled cells can be grouped by epi-state as follows.
