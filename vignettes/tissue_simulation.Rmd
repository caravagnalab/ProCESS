...
which allows to program the tissue evolution over time, adding new cells
...
Class `TissueSimulation` exports a `TissueSimulation$show()` method to get
...
```{r}
# build a tissue simulation whose tissue has width 1200 and
# height 900
...
-   add the specifics of mutant `B` (via `TissueSimulation$add_mutant()`, as
    we did for `A`);
...
in the model. To facilitate the user, CLONES can avoid drift by setting a
