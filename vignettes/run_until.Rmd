...
`TissueSimulation$run_until()` to let the simulation evolve until
...
All above variables can be built by using the method `TissueSimulation$var()`.
...
or `switches`, `TissueSimulation$var()` returns
...
The method `TissueSimulation$run_until()` takes as the parameter a formula and
