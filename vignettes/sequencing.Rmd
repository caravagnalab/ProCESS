...
[this article](mutations.md), ProCESS can simulate
the sequencing of the samples in the forest and return the
observed data.
...
The function `simulate_seq()` returns a named list of two elements:
...
ProCESS provides functions to plot the Variant Allele
...
ProCESS can also save the simulated reads in the SAM format
...
[`samtools`](http://www.htslib.org) can be used to split the reads by sample.
...
The `simulate_seq()` parameter `output_dir` sets the SAM output directory. See
the following section for usage examples of this parameter.

### Sample Purity

The _sample purity_ represents the concentration of tumour cells in a sample.
More formally, it is the ratio of the number of tumour cells in the total
number of cells in the sample, whether tumour or normal (i.e., cells with
the sample to the germline and pre-neoplastic mutations).
...
of a sample whose cells have germline and pre-neoplastic
...
At the moment, there are only two natures implementing
sequencers: the `ErrorlessIlluminaSequencer` class and
the `BasicIlluminaSequencer` natures.
...
base in which the error occurs, from the base position in the
read and the read position on the genome.
...
allows multiple writes in the same directory.
