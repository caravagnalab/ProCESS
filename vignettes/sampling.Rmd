...
> *Disclaimer:* CLONES/ProCESS internally implements the probability
...
We consider a simple monoclonal model, without epigenetic state.
...
> box after application of `TissueSimulation$sample_cells()` are no longer
...
This is also reflected by `TissueSimulation$get_cells()`, which now will not
...
object of the S4 class `SampleForest`.
...
number of cells, it might be complicated to view the full forest, unless a
...
Since mutant start has been randomised by `TissueSimulation$choose_cell_in()`,
...
ProCESS provides a `TissueSimulation$search_sample()` function to sample
...
`TissueSimulation$search_sample()` will attempt a fixed number of times to
...
The saved sample forest can successively be load by using the function
`load_sample_forest()`.
