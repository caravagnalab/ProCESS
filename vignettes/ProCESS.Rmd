...
1. a discrete tissue is first simulated, with distinct clones defined by
   ...
   by a time-varying structure that allows easily modelling of complex
...
3. from the simulated phylogenetic history of the tumour it is possible to
   ...
   outputs that can be used for alignment and downstream analysis. Every
...
1. Initialisation: a rectangular tissue is initialised, some species are defined, and a
   few cells placed onto it;
2. Tissue evolution: all the cells on the tissue are left to grow stochastically;
...
3. Mutational engine: an engine to simulate mutations is built using a real
   reference genome.
4. Map mutations: mutations are stochastically attached to the simulated
   phylogeny.
...
2. SAM: from the same mutations it is possible to generate reads for a tumour
   matched-normal assay, and fed that data to a custom bioinformatics
   pipeline.
